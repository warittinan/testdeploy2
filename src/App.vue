<template>
  <div class="w-screen h-screen">
    ...
    <div v-if="!gameStr" class="h-screen bg-gray-400 flex flex-col bg-[url('/bg/gg.gif')]"
      ...
      <!--Screen-->
      <div v-if="startBtn && countdownzone " class="flex justify-end">
        <!--Back btn-->
      ...
      <div class="flex flex-col items-center " v-if="!bg">
        <!--Play Screen-->
        <!-- Max sp -->
        <img v-if="!startBtn && countdownzone " class=" max-lg:hidden" :class="startBtn && countdownzone ? '-mt-48' : ''" src="/bg/logo-3.png" style="width: 70em; height: fit-content;">
        <img v-if="startBtn && countdownzone " class=" max-lg:hidden" :class="startBtn && countdownzone ? '-mt-48' : ''" src="/bg/logo-3.png" style="width: 70em; height: fit-content;">
        <!-- Min sp -->
        <img v-if="!startBtn && countdownzone" class=" mt-48 lg:hidden" src="/bg/logo-3.png" style="width: 70em; height: fit-content;">
        <img v-if="startBtn && countdownzone" class="  lg:hidden" src="/bg/logo-3.png" style="width: 70em; height: fit-content;">
          ...
          <div class="flex flex-col space-y-10 max-lg:hidden -mt-10 " v-if="startBtn && !gameStr && !cross && countdownzone">
            ...
            <div v-if="startBtn && countdownzone" class="rounded-2xl bg-white">
              <button class="btn hover:bg-yellow-400 border-transparent hover:border-transparent m-2 bg-yellow-500 w-32"
                @click="[countdown(), diffical('normal')]">
            ...
            <div v-if="startBtn && countdownzone" class="rounded-2xl bg-white">
              <button class="btn hover:bg-red-500 border-transparent hover:border-transparent m-2 bg-red-800 w-32"
                @click="[countdown(), diffical('hard')]">
                <span class="text-slate-50 text-font text-xl">hard</span>
              </button>
            </div>
          </div>
          <!-- Min sp -->
          <div class="flex flex-col space-y-10 lg:hidden" v-if="startBtn && !gameStr && !cross && countdownzone">
            <div class="rounded-2xl p-4 border-8 bg-black">
              <h1 class="text-slate-50 text-font text-3xl font-bold text-center">
                LEVEL
              </h1>
      ...
      <div v-show="!countdownzone" class="h-screen w-screen bg-[url('/bg/zz1.jpg')]" style="background-size: 100% 100%;">
        <div class=" flex justify-center h-full w-full content-center ">
          <div class="text-font flex items-center text-9xl text-white">
            {{ countdowntime }}
          </div>
        </div>
      </div>
      <!-- ===================================================================================================================== -->
      <div v-if="cross" class="h- w-full ">
        <!--Crosshair Screen-->
         <!-- Max sp --> 
    ...
    <div v-if="gameStr">
      <div :class="CROSSHAIR" >
        <div v-if="!gameOver" class="h-screen bg-[url('/bg/zz1.jpg')]" style="background-size: 100% 100%;"
          ...
          <div class="flex flex-col" >
            <div v-if="gameStr && !gameOver" class="flex justify-center">
              <!--Timer-->
              <div class=" bg-black m-4 p-3 border-8 rounded-2xl  " >
                <h1 class="text-white text-font ">Timer : {{ x }}</h1>
              </div>
              <div class="bg-black m-4 p-3 border-8 rounded-2xl " >
                <h1 class="text-white text-font ">Hit : {{ scoreHit }}</h1>
              </div>
            </div>
          </div>
        </div>
      ...
      <div v-if="gameOver" class="flex flex-col max-d:hidden">
        <div class="h-screen bg-[url('/bg/qq.gif')] ">
          <div class="flex justify-center ">
            <h class="text-font rounded-3xl mt-24 p-8 border-8 bg-black text-white text-3xl px-10">Results</h>
          </div>
          <div class="h-full flex flex-row justify-around items-end -mt-72">
            ...
            <div class="flex flex-col space-y-20">
              <div class="w-60 h-60 flex justify-center bg-white rounded-2xl">
                ...
                <div class="absolute m-32  ">
                  <h1 class="text-font  text-6xl font-bold text-black">10</h1>
                </div>
              </div>
              <div class="w-60 h-60 flex justify-center bg-white rounded-2xl">
                <div class="w-48 h-20 -mt-7 bg-black rounded-2xl">
            ...
            <div class="flex flex-col items-start space-y-10">
              <div class="w-80 h-72 mb-32 flex justify-center bg-white rounded-2xl">
                <div class="w-48 h-20 -mt-7 bg-black rounded-2xl">
                  <h1 class="text-font text-3xl text-center items-center p-4 text-white">
                    SCORE
                  </h1>
       ...
       <div v-if="gameOver" class="flex flex-col d:hidden">
        <div class="h-screen bg-[url('/bg/qq.gif')] ">
          ...
          <div class="h-full flex flex-row justify-around items-end -mt-80">
            <div class="flex flex-col items-start space-y-8">
              <div
                class="text-font btn rounded-2xl p-4 hover:border-white hover:bg-red-500 bg-black border-white border-8 h-20 w-50 text-center text-l pointer-events-auto"
                @click="reset">
                BACK TO MENU
              </div>
            </div>
            <div class="flex flex-col space-y-20">
              <div class="w-40 h-40 flex justify-center bg-white rounded-2xl">
                ...
                <div class="absolute m-16">
                  <h1 class="text-font text-3xl font-bold text-black">10</h1>
                </div>
              </div>
              <div class="w-40 h-40 flex justify-center bg-white rounded-2xl">
                <div class="w-26 h-16 -mt-7 bg-black rounded-2xl">
...
function countdown() {
  countdownzone.value = false;
  let time = setInterval(() => {
    --countdowntime.value;
    if(countdowntime.value == 0) {
      gameStart()
      countdownzone.value = true;
...
function hit() {
  scoreHit.value++;
  enemy.value = false;
  scrore.value = Math.ceil((scoreHit.value / clickT.value) * 100*acc.value);
}
function randomPosition() {
  targetTop.value = `${Math.floor(Math.random() * (winHeight.value - 500) +200)}`;
  targetleft.value = `${Math.floor(Math.random() * (winWidth.value - 500) +200)}`;
  enemy.value = true;
}
function delay(time) {
